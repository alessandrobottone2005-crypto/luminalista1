...
describe("countdown", () => {
  it("handles Rome offset and never goes negative", () => {
    const date = "2026-10-28T08:00:00+01:00";
    expect(getCountdown(date, Date.parse("2026-10-27T07:00:00Z"))).toEqual([
      1, 0, 0, 0,
    ]);
    expect(getCountdown(date, Date.parse("2026-10-29T07:00:00Z"))).toEqual([
      0, 0, 0, 0,
    ]);
  });

...
const page = `<main class="static-page"><header><img src="/Logo.svg" alt="Lumina — Lista 1" width="260"/><p>Liceo Gentileschi · Napoli</p></header><section><h1>La luce rende visibili le idee.</h1></section><section id="static-candidates"><h2>I candidati</h2>${candidates.map((c) => `<article><img src="${c.image}" alt="Ritratto di ${escape(c.name)} ${escape(c.surname)}" width="300" height="300" loading="lazy"/><h3>${escape(c.name)} ${escape(c.surname)}</h3><p>Classe ${escape(c.className)}</p></article>`).join("")}</section><section id="static-program"><h2>Il programma</h2>${programPoints
  .map(
    (p) =>
      `<article><h3>${p.id}. ${escape(p.title).replace("\n", " ")}</h3>${p.text
        .split("\n\n")
